...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert (sigmoid(a + 1e-7) - sigmoid(a - 1e-7)) >= 0.0
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    assert lt(a, b) if lt(a, c) and lt(c, b) else True
    assert lt(b, a) if lt(b, c) and lt(c, a) else True
    assert lt(b, c) if lt(b, a) and lt(a, c) else True
    assert lt(c, b) if lt(c, a) and lt(a, b) else True
    assert lt(c, a) if lt(c, b) and lt(b, a) else True
